...
--        ,beam.active_user as "MoonBeam"
    from ethereum_active_user eth
--    left join
--    bsc_active_user bsc
--    on eth.on_date = bsc.on_date
    left join
    polygon_active_user matic
    on eth.on_date = matic.on_date
    left join
    avalanche_active_user avax
    on eth.on_date = avax.on_date
    left join
    arbitrum_active_user arb
    on eth.on_date = arb.on_date
    left join
    optimism_active_user op
    on eth.on_date = op.on_date
    left join
    fantom_active_user ftm
    on eth.on_date = ftm.on_date
    left join
    celo_active_user celo
    on eth.on_date = celo.on_date
--    left join
--    moonriver_active_user river
--    on eth.on_date = river.on_date
--    left join
--    moonbeam_active_user beam
--    on eth.on_date = beam.on_date
)
